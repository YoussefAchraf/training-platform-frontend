...
export function InstructorDashboard() {
  ...
  useAppBadge(badgeUpcomingCount + badgeExpiringCertsCount + unreadMessagingCount);
  ...
  return (
    <BentoGrid>
      ...
      <StatTile
        id="tour-stat-completed"
        className={bentoStyles.span2}
        label={t('InstructorDashboard.completedSessions')}
        value={completedCount}
        icon={CheckCircle2}
        tone="success"
      />
      <StatTile
        id="tour-stat-upcoming-accepted"
        className={bentoStyles.span2}
        label={t('InstructorDashboard.upcomingAccepted')}
        value={upcomingCount}
        icon={CalendarCheck2}
        tone="info"
      />
      ...
      {/* Not in the original mockup (which only has "Your sessions"), kept as
          its own card because it's real, working functionality with its own
          guided-tour step (#tour-card-recently-completed) - removing it would
          regress existing functionality and break that step. */}
